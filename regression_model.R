...
#Predications on test data (best model)
...
new_data <- data.frame(
  Brand = factor("BMW"),  
  ModelYear = 2020,        
  Engine = factor("Diesel"),
  Miles = 15000,
  Gears = factor("Automat"),
  Dealer = factor("Corporate Dealers")
)

str(new_data)

#Adjust new data to match the levels in the cleaned data
levels_data <- clean_data
new_data$Brand <- factor(new_data$Brand, levels = levels(levels_data$Brand))
new_data$Engine <- factor(new_data$Engine, levels = levels(levels_data$Engine))
new_data$Gears <- factor(new_data$Gears, levels = levels(levels_data$Gears))
new_data$Dealer <- factor(new_data$Dealer, levels = levels(levels_data$Dealer))

x_new_data <- model.matrix(~ . -1, new_data)

str(x_new_data)

predicted_values <- predict(model_5, s = lasso_lambda, newx = x_new_data)

new_data$Predicted_LogPrice = predicted_values

new_data$Predicted_Price = exp(predicted_values)
